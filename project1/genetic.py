...
def mutation(population, mut_rate):
    ...
    for i in range(pop_size):
        if random.random() < mut_rate:
            ...
            population[i, mut_point] = random.randint(0, 1)
    return population
...
def genetic(pop_size, num_genes, gene_range, cross_rate, mut_rate, select_rate, num_generations):
    ...
    for i in range(num_generations):
        ...
        print('Generation: ', i, ' Best Fitness: ', best_fitness[i], ' Average Fitness: ', average_fitness[i])
    end_time = time.time()
...
def test():
    ...
    select_rate = 0.2
    num_generations = 100
    best_individual, best_fitness, average_fitness, run_time = genetic(pop_size, num_genes, gene_range, cross_rate, mut_rate, select_rate, num_generations)
...
def test(df, cols):
    pop_size = 1000
    num_genes = len(cols) * (len(cols) - 1) / 2
    gene_range = [0, 2]
    cross_rate = 0.8
    mut_rate = 0.01
    select_rate = 0.2
...
# Output: graph
def create_graph_from_gene(individual, cols):
    graph = nx.DiGraph()
